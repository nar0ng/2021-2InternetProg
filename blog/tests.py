...
class TestView(TestCase):
    ...
    def navbar_test(self, soup):
        ...
        self.assertEqual(logo.attr['href'],'/')
        home = navbar.find('a',text='Home')
        self.assertEqual(home.attr['href'],'/')
        blog = navbar.find('a',text='Blog')
        self.assertEqual(blog.attr['href'],'/blog/')
        about = navbar.find('a',text='About Me')
        self.assertEqual(about.attr['href'],'/about_me/')
...
def test_post_list(self):
    ...
    # 포스트(게시물)이 하나도 없는 경우
    self.assertEqual(Post.objects.count(),0) #포스트 오브젝트수가 0이냐?
    # 적절한 안내 문구가 포함되어 있는지
    main_area = soup.find('div', id='main-area') #main-area라는 아이디를 가진 div 태그 찾기
